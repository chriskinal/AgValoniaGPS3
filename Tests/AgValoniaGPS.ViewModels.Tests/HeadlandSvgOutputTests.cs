...
public class HeadlandSvgOutputTests
{
    ...
    public void UserField_HeadlandWithBoundaryOffset_SvgOutput()
    {
        ...
        // Tram boundary tracks
        if (tramService.OuterBoundaryTrack.Count > 1)
            svg.AddPolyline(tramService.OuterBoundaryTrack, "#c03030", 1.5, "6,3");
        if (tramService.InnerBoundaryTrack.Count > 1)
            svg.AddPolyline(tramService.InnerBoundaryTrack, "#3030c0", 1.5, "6,3");
        ...
        svg.AddLabel("Red dashed: outer tram track", 10, 74, "#c03030");
        svg.AddLabel("Blue dashed: inner tram track", 10, 92, "#3030c0");
    ...
    [Test]
    public void UserField_TramLinesWithBoundaryClipping_SvgOutput()
    {
        var boundary = LoadBoundary();
        Assert.That(boundary.Count, Is.GreaterThan(10));
        ...
        tramService.SetBoundaryFence(boundary);
        tramService.GenerateParallelTramLines(track, 600);

        // Render SVG
        var allPts = boundary.Select(p => (p.Easting, p.Northing));
        var svg = new SvgRenderer(allPts);
        svg.AddPolygon(boundary, "#d4760a", 2);

        // Track
        var trackLine = track.Points.Select(p => new Vec2(p.Easting, p.Northing)).ToList();
        svg.AddPolyline(trackLine, "#3366ff", 1.5);

        // Parallel tram lines
        foreach (var line in tramService.ParallelTramLines)
            svg.AddPolyline(line, "#c06080", 0.8);

        svg.AddLabel($"Orange: boundary ({boundary.Count} pts)", 10, 20, "#d4760a");
        svg.AddLabel($"Blue: AB track", 10, 38, "#3366ff");
        svg.AddLabel($"Pink: {tramService.ParallelTramLines.Count} tram lines", 10, 56, "#c06080");

        var svgPath = Path.Combine(OutputDir, "tram_parallel_lines.svg");
        svg.Save(svgPath);
        TestContext.WriteLine($"SVG saved: {svgPath}");
        TestContext.AddTestAttachment(svgPath, "Tram parallel lines");
    }

    [Test]
    public void SquareField_HeadlandAndTram_SvgOutput()
    {
        var boundary = CreateSquare(200);
        ...
        // Tram tracks
        var offsetService = new TramLineOffsetService();
        var tramService = new TramLineService(offsetService, NullLogger<TramLineService>.Instance);
        ConfigurationStore.Instance.Tram.TramWidth = 12.0;
        ConfigurationStore.Instance.Vehicle.TrackWidth = 1.8;
        ConfigurationStore.Instance.Tool.Width = 6.0;

        if (headland != null) tramService.GenerateBoundaryTramTracks(headland);

        var track = new Models.Track.Track
        {
            Name = "AB", Points = new List<Vec3> { new(100, 0, 0), new(100, 200, 0) },
            Type = Models.Track.TrackType.ABLine
        };
        tramService.SetBoundaryFence(boundary);
        tramService.GenerateParallelTramLines(track, 200);

        // Render SVG
        var allPts = boundary.Select(p => (p.Easting, p.Northing));
        var svg = new SvgRenderer(allPts, 600);
        svg.AddPolygon(boundary, "#d4760a", 2);
        if (headland != null) svg.AddPolygon(headland, "#c8b400", 2);

        if (tramService.OuterBoundaryTrack.Count > 1)
            svg.AddPolyline(tramService.OuterBoundaryTrack, "#c03030", 1, "4,2");
        if (tramService.InnerBoundaryTrack.Count > 1)
            svg.AddPolyline(tramService.InnerBoundaryTrack, "#3030c0", 1, "4,2");

        var trackLine = track.Points.Select(p => new Vec2(p.Easting, p.Northing)).ToList();
        svg.AddPolyline(trackLine, "#3366ff", 1);

        foreach (var line in tramService.ParallelTramLines)
            svg.AddPolyline(line, "#c06080", 0.6);

        svg.AddLabel("Square field: boundary + headland + tram", 10, 20);

        var svgPath = Path.Combine(OutputDir, "square_field_complete.svg");
        svg.Save(svgPath);
        TestContext.WriteLine($"SVG saved: {svgPath}");
        TestContext.AddTestAttachment(svgPath, "Square field complete");
    }
}
